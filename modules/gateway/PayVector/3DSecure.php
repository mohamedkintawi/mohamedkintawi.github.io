...
<html>
	...
	<body onload="onLoadHandler();" style="margin-left: auto; margin-right: auto;">
		<div name="3DS" style="margin-left: auto; margin-right: auto;">
			<form name="processform" method="post" action="<?php echo $GLOBALS['session']->get('ACSUrl', 'payvector') ?>" />
			<input type="hidden" name="PaReq" value="<?php echo $GLOBALS['session']->get('PaREQ', 'payvector') ?>" />
			<input type="hidden" name="TermUrl" value="<?php echo $GLOBALS['session']->get('TermUrl', 'payvector') ?>" />
			<input type="hidden" name="MD" value="<?php echo $GLOBALS['session']->get('CrossReference', 'payvector') ?>" />
			<noscript>
