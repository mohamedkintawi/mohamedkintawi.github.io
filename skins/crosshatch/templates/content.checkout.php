<div id="checkout">
	...
	<form action="{$VAL_SELF}" method="post" enctype="multipart/form-data" id="basket">
		 {if $INCLUDE_CHECKOUT} {include file='templates/content.checkout.confirm.php'} {/if}
		...
		<div class="basket_product">
			...
			<p>
				<a href="{$item.link}" class="txtDefault"><strong>{$item.name}</strong></a> {if !empty($item.product_code)}- {$item.product_code}{/if} {if $item.base_price_display}({$item.base_price_display}){/if}
			</p>
			 {if $item.options} {foreach from=$item.options item=option}
			<p>
			...
			<p>
				...
				{$LANG.common.quantity}: <input name="quan[{$hash}]" type="text" value="{$item.quantity}" class="quantity" {$quan_read_only}/>
			</p>
		</div>
		 {/foreach}
		<div class="subtotals">
			<p>
				<span class="price">{$SUBTOTAL}</span>{$LANG.basket.total_sub}
			</p>
			 {if isset($SHIPPING)}
			<p>
			 ...
			 {/if} {foreach from=$TAXES item=tax}
			<p>
				<span class="price">{$CUSTOMER_LOCALE.mark} {$tax.value}</span>{$tax.name}
			</p>
			 {/foreach}
		</div>
		<div class="discounts">
			...
			<p>
				<span class="price">{$coupon.value}</span><a href="{$VAL_SELF}&amp;remove_code={$coupon.remove_code}" title="{$LANG.common.remove}">{$coupon.voucher}</a>
			</p>
			 {/foreach} {if isset($DISCOUNT)}
			<p>
				<span class="price">{$DISCOUNT}</span>{$LANG.basket.total_discount}
			</p>
			 {/if}
		</div>
		 {if $BASKET_WEIGHT}
		<p>
			<strong>{$LANG.basket.weight}:</strong> {$BASKET_WEIGHT}
		</p>
		{/if} {if $CUSTOMER_LOCALE.description}
		<p>
			{$CUSTOMER_LOCALE.mark} {$LANG.basket.unconfirmed_locale}{/if}
			<div class="total">
				<span class="price">{$TOTAL}</span> {$LANG.basket.total_grand}
			</div>
			<div class="basket_actions">
				<a href="{$STORE_URL}/index.php?_a=basket&amp;empty-basket=true" class="btn button_submit left">{$LANG.basket.basket_empty}</a>
				<input type="submit" name="update" class="btn button_submit update" value="{$LANG.basket.basket_update}"/>
				{if $DISABLE_CHECKOUT_BUTTON!==true} <input type="submit" name="proceed" class="btn button_submit" value="{$CHECKOUT_BUTTON}"/>
				{/if}
			</div>
		</form>
		 {if $CHECKOUTS}
		<p class="alternate_checkout">
			-- {$LANG.common.or} --
		</p>
