...
<p>
	{$LANG.newsletter.view_newsletter_archive}
</p>
<div id="myaccount">
	<ul>
 ...
 {else}
<p>
	{$LANG.newsletter.no_archived_newsletters}
</p>
 {/if}
<h2>{$LANG.newsletter.subscription}</h2>
 {if $IS_USER} {if $SUBSCRIBED}
<p>
	{$LANG.newsletter.customer_is_subscribed} <a href="{$URL.unsubscribe}">{$LANG.newsletter.click_to_unsubscribe}</a>
</p>
 {else}
<p>
	{$LANG.newsletter.customer_not_subscribed} <a href="{$URL.subscribe}">{$LANG.newsletter.click_to_subscribe}</a>
</p>
 {/if} {else}
<p>
...
<form action="{$VAL_SELF}" method="post" enctype="multipart/form-data">
	...
	<div>
		<input name="submit" class="button_submit" type="submit" value="{$LANG.form.submit}"/>
	</div>
</form>
	{/if}
{/if}