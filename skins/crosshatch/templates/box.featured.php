<div class="box">
	<form action="{$VAL_SELF}" method="post" class="top addForm" enctype="application/x-www-form-urlencoded">
		...
		<p class="price">
			<span class="price_previous">{$featured.price}</span><span class="price_sale">{$featured.sale_price}</span>
		</p>
		 {else}
		<p class="price">
			{$featured.price}
		</p>
		 {/if} {if $featured.ctrl_purchase && !$CATALOGUE_MODE}
		<div class="button">
			<input type="hidden" name="add[{$featured.product_id}][quantity]" value="1"/>
			<input type="submit" class="btn" value="{$LANG.catalogue.buy_now}"/>
		</div>
		 {/if}
	</form>
</div>