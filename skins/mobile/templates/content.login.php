<form action="{$VAL_SELF}" method="post">
  <h2>{$LANG.account.login}</h2>
	<fieldset>
	  ...
	  <div><span><input type="checkbox" name="remember" id="login-remember" value="1" class="" {if $REMEMBER}checked="checked" {/if} /></span>  <label for="login-remember">{$LANG.account.remember_me}</label></div>
	  <div><a href="{$STORE_URL}/index.php?_a=recover">{$LANG.account.forgotten_password}</a></div>
	</fieldset>
  <div class="buttons">
