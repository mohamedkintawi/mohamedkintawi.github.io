{if $IS_USER}
	...
	<p>
	  ...
	  <select name="delivery_address" id="delivery_address" class="update_form textbox" >
...
{else}
	<div id="register">
	  ...
	  <fieldset id="address_billing"><legend>{$LANG.address.billing_address}{if !$ALLOW_DELIVERY_ADDRESS} {$LANG.address.ship_to_billing_only}{/if}</legend>
		...
		{if $TERMS_CONDITIONS}
		<div><span><input type="checkbox" id="reg_terms" name="terms_agree" value="1" {$TERMS_CONDITIONS_CHECKED} /> <a href="{$TERMS_CONDITIONS}" target="_blank">{$LANG.account.register_terms_agree}</a></span></div>
		{/if}
		{if $ALLOW_DELIVERY_ADDRESS}<div><span><input type="checkbox" name="delivery_is_billing" id="delivery_is_billing" {$DELIVERY_CHECKED} /> {$LANG.address.delivery_is_billing}</span></div>{/if}
	  </fieldset>
