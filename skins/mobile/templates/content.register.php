...
{include file='templates/content.recaptcha.php'}
	...
	{if $TERMS_CONDITIONS}
	<div><span><input type="checkbox" id="register-terms" name="terms_agree" value="1" {$TERMS_CONDITIONS_CHECKED} /> <a href="{$TERMS_CONDITIONS}" target="_blank">{$LANG.account.register_terms_agree}</a></span></div>
	{/if}
	<div><span><input type="checkbox" id="register-mailing" name="mailing_list" value="1" {if isset($DATA.mailing_list) && $DATA.mailing_list == 1}checked="checked"{/if} /> {$LANG.account.register_mailing}</a></span></div>
  </fieldset>
  <div class="buttons"><input type="submit" name="register" value="{$LANG.account.register}" class="button_submit" /></div>
</form>