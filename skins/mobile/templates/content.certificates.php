<form action="{$VAL_SELF}" method="post">
  ...
  <fieldset>
	...
	{/if}
	<div><label for="gc-message">{$LANG.common.message} {$LANG.common.optional}</label><span><textarea name="gc[message]" rows="5" id="gc-message" class="textbox">{$POST.message}</textarea></span></div>
  </fieldset>
  {if $ctrl_allow_purchase}
  <div class="buttons"><input type="submit" class="button_submit" name="Submit" value="{$LANG.catalogue.add_to_basket}" /></div>
  {/if}
</form>