{if isset($GATEWAYS)}
  ...
  <form id="gateway-select" action="{$VAL_SELF}" method="post" enctype="multipart/form-data">
	...
	<div class="buttons"><input type="submit" value="{$LANG.common.continue}" class="button_submit" /></div>
	  </form>
{/if}
...
<h2>{$LANG.gateway.amount_due}</h2>
  <p>{$LANG_AMOUNT_DUE}</p>
	{$FORM_TEMPLATE}
	{/if}
  <div class="buttons"><input type="submit" value="{$BTN_PROCEED}" class="button_submit" /></div>
